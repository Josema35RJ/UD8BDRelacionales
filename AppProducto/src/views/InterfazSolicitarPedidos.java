		...
		btnRegistrar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(model.getValueAt(0, 2).equals("")||model.getValueAt(0, 3).equals("")) {
					JOptionPane.showMessageDialog(InterfazSolicitarPedidos.this, "NO SE PUEDE INTRODUCIR VALORES NULOS","ERROR",JOptionPane.ERROR_MESSAGE);
				}else {
					int id=Integer.valueOf(ultimocompra()) + 1;
					String stringid=String.valueOf(id);
					java.sql.Date fecha = new java.sql.Date(Long.valueOf(model.getValueAt(0, 0).toString().replace("/", "")));
					java.sql.Date fechac = new java.sql.Date(fecha.getTime());
					Time hora = new java.sql.Time(Long.valueOf(model.getValueAt(0, 1).toString().replace(":", "")));
					java.sql.Time horac = new java.sql.Time(hora.getTime());
					int cantidad_stock=Integer.valueOf(String.valueOf(model.getValueAt(0, 3)));
					Compra c=new Compra(stringid,(Date) fechac,horac,il.User.getId_Usuario(),model.getValueAt(0, 2).toString(),cantidad_stock);
					...
					try {
						for(Producto x: os.getAllProducts(Conexion.obtener())) {
							if(x.getId_Producto().equals(c.getId_Producto())) {
								...
								JOptionPane.showMessageDialog(InterfazSolicitarPedidos.this, "PRODUCTO ACTUALIZADO Y COMPRA ACTUALIZADA");
								encontrado=true;
							}
						}
						if(encontrado==false) {
							JOptionPane.showMessageDialog(InterfazSolicitarPedidos.this, "PRODUCTO NO ENCONTRADO EN LA BASE DE DATOS");
						}
